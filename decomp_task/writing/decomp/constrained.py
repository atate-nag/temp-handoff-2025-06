...
from decomp_task.writing.sections import get_available_section_selector, sections_to_str, section_dict
import json
from typing import List
from decomp_task.writing.decomp.unconstrained import WritingTaskDecomp, WritingCondition, get_schema, create_decomposer
...
class ConstrainedWritingTaskDecomp(WritingTaskDecomp):
    ...
    def iterative(self, task:ConstrainedWritingTask)->None:
        if self.iterative_toggle==0:
            ...
            available_sections_type = get_available_section_selector(task.section_type, None, return_raw=True)
            return not( len(available_sections_type) == 1 and available_sections_type[0].name == "standard" )
        else:
            raise IndexError
    ...
    def _planning_decomp_single(self, message, task:ConstrainedWritingTask)->None:
        if self.iterative_toggle == -1:
            ...
            available_sections_type = get_available_section_selector(task.section_type, None, return_raw=False, single=True)
        ...
        if self.iterative_toggle == -1:
            planing_results = [ x|{"section_type": "standard" } for x in planing_results ]
    ...
    def _is_done(self, message, task:ConstrainedWritingTask):
        ...
        available_sections_type = get_available_section_selector(task.section_type, last_section, return_raw=True)
    ...
    def _get_next_sections_type(self, message, task:ConstrainedWritingTask):
        ...
        if type(available_sections_type) == str:
            ...
            return { "section_type" : available_sections_type[0].name, "section_type_reasoning": "Only one type remaining"}
        else:
        ...
        message = f"From available sub-section types: {available_types}; select the {'next' if task.child_task else 'first'} sub-section type." + \
        ...
        while most_common is None:
            ...
            if self.setup_config["llm_config"]["cache_seed"] is None:
                ...
                inter_llm_config["cache_seed"] = 10000*( retries + 1 ) + self.setup_config["llm_config"]["cache_seed"]
            ...
            if next_section_type['section_type'] in section_dict and \
                next_section_type['section_type'] in [ x.name for x in available_sections_type]:
                output[ next_section_type['section_type'] ] = output.get( next_section_type['section_type'], 0 ) + 1
    ...
    def _get_section_content(self, message, task:ConstrainedWritingTask):
        ...
        message =   f"Selected section type is '{next_section_type['section_type']}'.\n\n" + \
...
class ConstrainedWritingCondition(WritingCondition):
    ...
    def __call__(self, task:ConstrainedWritingTask ):
        available_types = get_available_section_selector(task.section_type, return_raw=True)
        ...
        if "Not a valid parent section name" in available_types:
            ...
            return { "decompose" : False, "reasoning" : "This section cannot be further decomposed per definition." }
        elif task.section_type in [ "strategy recommendation" , "main body" ]:
            return { "decompose" : True, "reasoning" : "This section must be further decomposed per user." }
...
def create_single_step_decomposer():    
    ...
    return {"name": "single_decomp", "system_message" : system_message, "summary_method" : "reflection_with_llm",  "summary_args":  { "summary_prompt": summary_prompt } } , schema
...
def create_next_section_type_agent():
    ...
    schema = {
        "$schema": "http://json-schema.org/draft-04/schema#",
        "type": "object",
        "properties": {"section_type": {"type": "string"},"section_type_reasoning": {"type": "string"}},
