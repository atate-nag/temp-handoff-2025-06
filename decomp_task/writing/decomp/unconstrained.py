...
class WritingCondition(BaseCondition):
    def __init__(self, **other_args) -> None:
        super().__init__(
            user_system_prompt="You are a helpful assistant. Your role is to determine if a section should be decomposed into sub-sections."
...
def get_schema(add_section_type=False, ret_section=True):
    ...
    if add_section_type:
        ...
        st_description = (
            "The most appropriate type from the available section types for the sub-section."
        ...
        schema["properties"]["detailed_section_content"]["description"] = (
            "using the section type definition and parent section expected content "
            + schema["properties"]["detailed_section_content"]["description"]
        )
        schema["properties"]["section_title"]["description"] = (
            schema["properties"]["section_title"]["description"]
    ...
    if not ret_section:
        schema = {
            "type": "object",
            "properties": {"sections": {"type": "array", "items": schema}},
            "required": ["sections"],
            "additionalProperties": False,
        }
...
def create_decomposer():
    ...
    return {
        "name": "decomp",
        "system_message": base + end,
        "summary_method": "reflection_with_llm",
        "summary_args": {"summary_prompt": summary},
    }, schema  #
