...
class TaskGraph:
    ...
    def add_task(self, task , keys ):
        attributes = ",\n".join([ f"                    task.{key} = $taskParams.{key}" for key in keys ])
    ...
    def update_task(self, task, update_keys):
        ...
        attributes = ",\n".join([ f"                    task.{key} = $taskParams.{key}" for key in update_keys if key not in ["task_id", "root_task_id"]])
        update_task = f"""
