...
class Condenser(BaseAgents):
    def __init__(self, **other_args):
        super().__init__(
            user_system_prompt="Your task is write 1 to 2 paragraph summary on [Topic], using the provided text chucks. "
            + "You can only use the provided text chunks to write the paragraphs. ",
            **other_args,
        )
...
class IndustryExtractorOnline(BaseAgents):
    def __init__(
        self, gics_code: str, graph_extractor: GraphExtractorOnline, **other_args
    ):
    ...
    def extract_raw_trends(self):
        ...
        trends = [
            (
                rec[keys[0]]["Description"],
                np.array(rec[keys[0]]["DescriptionEmbedding"]),
            )
            for rec in records
        ]
    ...
    def get_embedding(self, topic: str):
        ...
        return np.array(
            client.embeddings.create(model="text-embedding-ada-002", input=topic)
            .data[0]
            .embedding
        ).reshape((1, -1))
...
class CompanyExtractorOnline(BaseAgents):
    def __init__(
        self, company: str, graph_extractor: GraphExtractorOnline, **other_args
    ):
        super().__init__(**other_args)
        self.ge = graph_extractor
    ...
    def extract_raw_insights(self):
        ...
        insights = [
            (
                rec[keys[0]]["description"],
                np.array(rec[keys[0]].get("descriptionEmbedding", [])),
                rec[keys[0]]["insightId"],
                rec[keys[0]]["source"],
            )
            for rec in records
        ]
    ...
    def _add_embeddings(self):
        ...
        for i, (r, emb) in enumerate(self.raw_insights):
            if len(emb) == 0:
                emb = np.array(
                    client.embeddings.create(model="text-embedding-ada-002", input=r)
                    .data[0]
                    .embedding
                )
...
class WritingTaskExecuter(BaseTaskExecuter):
    def __init__(
        self,
        company,
        company_extractor: CompanyExtractorOnline = None,
        industry_extractor: IndustryExtractorOnline = None,
        database: str = "neo4j",
        uri: str = os.getenv("NEO4J_URL"),
        user: str = os.getenv("NEO4J_USER"),
        password: str = os.getenv("NEO4J_PASSWORD"),
        **other_args,
        ...
        self.company = company
        graph_extractor = GraphExtractorOnline(
            database=database, uri=uri, user=user, password=password
        )
        self.company_extractor = (
            CompanyExtractorOnline(
                company, graph_extractor, llm_config=other_args["llm_config"]
            )
            if company_extractor is None
            else company_extractor
        )
    ...
    def _prepare_company_background_info(self, task: WritingTask):
        ...
        paragraphs = Condenser(
            ...
            constraint,
            [r[0] for r in self.company_extractor.raw_insights],
            provided_text,
        )
    ...
    def _generate_message(self, task: WritingTask):
        ...
        indent = "   "
        constraints_str = "\n".join(
            [indent + "    " + constraint for constraint in task.constraints]
        )

        current_workflow = task.root_task.workflow(
            lambda x: f"{x.name}:{x.description}", as_string=False
        )[1:]
...
def create_writer():
    paragraph_system_prompt = (
        "You are a brilliant strategy consultant, fantastic at writing a business report in collaboration with other consultants."
