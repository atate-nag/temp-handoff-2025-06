...
import uuid
WorkFlowDict = TypedDict('WorkFlowDict', {'prefix': List[int], 'string': str, 'num': int})

class WritingTask(BaseTask):
    def __init__(self:Self, name:str, description:str, objectives:list, 
                 constraints:list, root_task:Self, id_str:str = None,
                 previous_task:Self=None, parent_task:Self=None, predict:bool = True )-> None:
        super().__init__(
            name = name,
    ...
    def id(self):
        if self.id_str:
            ...
            name = self.name + str(self.parent_task.id if self.parent_task else self.previous_task.id)
    ...
    def task_details_str(self:Self, indent=0)-> str:
        ...
        indent =    "    "*indent
        message = f"{indent}Section Title: '{self.name}'\n" + \
    ...
    def _get_workflow(self:Self, fns:Callable=None, num:int=1, output:List[WorkFlowDict|None]=[], prefix:List[int|None]=[]) -> List[WorkFlowDict]:        
        inter = {
            "prefix"    : prefix,
        ...
        if self.child_task is not None:
            output = self.child_task._get_workflow(fns=fns, num=1, output=output, prefix=prefix+[num])

        if self.next_task is not None:
            output = self.next_task._get_workflow(fns=fns, num=num+1, output=output, prefix=prefix)
        
        return output
    
    def workflow(self:Self, fns:Callable=None, as_string:bool=True)->List[WorkFlowDict]|str:
    ...
    def create_task(self, overriding_args:dict, parent_or_previous:bool)->Self:
        ...
        parent_task, previous_task = (self, None) if parent_or_previous else (None, self)
...
class ConstrainedWritingTask(WritingTask):
    ...
    def task_details_str(self:Self, indent:int=0)->str:        
        indent =    "    "*indent
        constraints_str = '\n'.join([indent+ '    ' + constraint for constraint in self.constraints])
        message = f"{indent}Section Title: '{self.name}'\n" + \
...
class WritingTaskManager(BaseTaskManager):
    def __init__(self:Self, root_task:WritingTask|ConstrainedWritingTask, 
                 task_decomposer, condition, executer, threshold:int=2, 
                 max_workers:int = 4,
                 **other_args):                
        super().__init__(root_task, task_decomposer, condition, executer, threshold=threshold, **other_args)
        ...
        while len(tasks) <= self.threshold:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for current_task in current_tasks:
                    jobs.append(executor.submit(lambda x : x.decompose(self.task_decomposer, self.condition), *(current_task,)))
            ...
            for current_task in current_tasks:
                if current_task.child_task is not None:
                    next_tasks = next_tasks + current_task.child_task._get_subsequent_tasks()
        ...
        while len(unvisited):
            ...
            if current_task is not self.root_task:
                if current_task.parent_task:
                    ...
                    task_graph.link_sibling_tasks(current_task.previous_task, current_task)
                else:
    ...
    def create_document(self, file_name):        
        ...
        doc = Document(f"{file_name}_report", page_numbers=True, geometry_options=geometry_options)
    ...
    def _fill_document(self, doc:Document, task:WritingTask|ConstrainedWritingTask):
        ...
        all_cites = [ f"[{num}] - Source: {self.executer.company_extractor.raw_insights[num][3]}" for num in all_cites ]
...
def list_to_report(current_workflow):
    ...
    for i,task_str in enumerate(current_workflow):
        num = task_str["prefix"] + [ task_str["num"] ]
        current_workflow[i] = indent*(len(num)-1) + ".".join([f"{n}" for n in num]) + ") " + \
             task_str["string"][0] + "\n\n" + "\n".join([ indent*(len(num)-1) + r for r in task_str["string"][1].splitlines() ])
    current_workflow = "\n".join(current_workflow)
    return current_workflow
