...
dotenv.load_dotenv()
from decomp_task.writing import (
    WritingTaskManager,
...
def create_params(
    company: str, temperature: float, run_name: str, with_cache=True, with_execute=False
) -> Tuple[Dict, ConstrainedWritingTask, WritingTaskManager, TaskGraph, str]:

    llm_config = {
        "config_list": [{"model": "gpt-4o", "api_key": os.environ["OPENAI_API_KEY"]}],
        "temperature": temperature,
        "cache_seed": 43 if with_cache else None,
    }
    ...
    args = {
        ...
        "objectives": [
            f"A business strategy report that answers the strategy problem statement '{question}'."
        ],
        "constraints": [
            f"The report should discuss: '{context}' ",
            "main body must include strategy recommendation among other things",
        ],
        "root_task": None,
        "id_str": file_name,
    }
    ...
    decomposer = ConstrainedWritingTaskDecomp(
        company=company, iterative=-1, llm_config=llm_config
    )
...
def run_decomp(
    ...
    args, root, task_manager, task_graph, file_name = create_params(
        company, temperature, run_name, with_cache, with_execute
    )

    """Create Task Plan"""
    error_message = ""
    try:
        root.execution_output = task_manager.get_predicted_plan(
            lambda task: f"{task.name}:{task.id}"
        )
        if with_execute:
    ...
    if len(error_message) > 0:
        output_message = (
            error_message
            + " You may still be able to view the intermediate generated task (if chosen) at Neo4j using the query below"
        )
    else:
