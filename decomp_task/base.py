...
def create_agents(llm_config, agents_config, is_termination_msg, code_execution_config= False  ):
...
class BaseAgents():
    def __init__(
        ...
        is_termination_msg = is_termination_msg if is_termination_msg else self.termination_condition_autogen
        if type(user_system_prompt) == UserProxyAgent:
            user = user_system_prompt
        else:
            user = create_user(llm_config=llm_config, is_termination_msg=is_termination_msg, system_prompt=user_system_prompt)
        ...
        if len(agents_config) or agents_config is not None:
            self.agents_config = create_agents(llm_config=llm_config, is_termination_msg=is_termination_msg, agents_config=agents_config)
    ...
    def extract_json(self, message, schema):
        ...
        llm_config = {"config_list": [{"model": "gpt-3.5-turbo-0125", "api_key": os.environ["OPENAI_API_KEY"]}] , "temperature":0}
    ...
    def convo_with_structure(self, agent_name, message, structure_schema=None ):
        agent = [ agent for agent in self.agents_config if agent["name"] == agent_name ][0]
        ...
        if chat_message["summary_method"] == "reflection_with_llm" and "summary_args" in agent:
            chat_message = chat_message | {  "summary_args":  agent["summary_args"]} # { "summary_prompt": summary_prompt }
...
class BaseTask():
    def __init__(self, name:str, description:str, objectives:list, constraints:list, root_task:Any, id_str:str = None)->None:
        self.name = name
        self.description = description
        self.objectives = objectives        
        self.constraints = constraints
        self.root_task = root_task
        self.is_decompose = None if root_task else { "decompose": True , "reasoning": "Root must always be decomposed"}
        self.execution_output = []
        self.id_str = id_str
    
    @property
    def id(self):
        return uuid.uuid5(uuid.NAMESPACE_OID, str(type(self)) + (self.id_str if self.id_str else self.name) )
...
class BaseTaskManager(Generic[GenericTask, GenericCondition, GenericTaskDecomp, GenericTaskExecuter]):
    def __init__(self, 
