...
class AgentStateManager:
    ...
    def __init__(self, agent):
        self.agent = agent
        self.machine = Machine(model=self, states=['INITIAL', 'INITIALISED', 'LOADED', 'RUNNING'], initial='INITIAL')
    ...
    def validate_input_files(self, input_files, prompt, agent_id):
        dprint(f"Validating input files {input_files} with agent_id {agent_id} and client = {self.client}")
        try:
            validated_input = AgentInputFilesModel(client=self.client,input_files=input_files, agent_id=agent_id, prompt=prompt)
            print("Input validated successfully.")
            return True
        except ValidationError as e:
            print(f"Validation error: {e}")
