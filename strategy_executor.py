...
if not DEBUG_RUN:
    ...
    def trend_analysis(client, market_insights):
        trends_file = overseer_manage_assistant(
            client, run_trends_analysis, market_insights, company_data
        )
        return trends_file

    def capabilities_analysis(client, company_insights):
        capabilities_file = overseer_manage_assistant(
            client, run_capabilities_analysis, company_insights, company_data
        )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        future_capabilities = executor.submit(
            capabilities_analysis, client, company_insights
        )
    ...
    challenges_file = overseer_manage_assistant(
        client, run_challenges_analysis, trends_file, capabilities_file
    )
    ...
    actions_file = overseer_manage_assistant(
        client, run_actions, top_challenge, trends_file, capabilities_file
    )
