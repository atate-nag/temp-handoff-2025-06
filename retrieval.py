...
def retrieve_docs(input_folder, output_folder, threshold=.7):
    folders = [input_folder]
    files_ = []
    for folder in folders:
        files_location = folder 
        print(f"\nfiles_location: {files_location}\n")
        # print(f"\nfiles: {list(sharePointRetriever.get_all_documents_from_folder(files_location,extension='pdf'))}\n")
        files_.extend(
            list(
                sharePointRetriever.get_all_documents_from_folder(
                    files_location, extension=["pdf",'pptx','docx']
                )
            )
        )
    files = []
    ...
    for file in files:
        ...
        doc =  Document(page_content=parsed["content"],metadata={'source':file})#TODO metadata=parsed["metadata"])
    ...
    researches = [["NAG","Numerical Algorithms Group",
                   ...
                   "leadership", "Services"]]
    for query in ["Can you tell me more about NAG (Numerical Algorithm Group) insights, perspective, risks, opportunities, and challenges?",
        ...
        result_steps, result_response, result_thread  = query_assistant(client, queries_agent['id'],query )
        ...
        for message in messages:
            if message.role=='assistant':
                queries.append(message.content[0].text.value.split('\n'))
    ...
    #               ["NAG insights"],
    #               ["Markets",]]
    for research, name in zip(researches, ['Files-DATA-retrieved','Files-INSIGHT-retrieved','Files-MARKET-retrieved']):
        retrieved_docs = []
        for queries in research:
        ...
        files_to_remove = [os.path.join(name,f) for f in os.listdir(name) if os.path.isfile(os.path.join(name, f))]
        for f in files_to_remove:
            os.remove(f)
        for file in retrieved_docs:
            print(os.path.join(name,file.split("/")[-1]))
            shutil.copyfile(file, os.path.join(name,file.split("/")[-1]))
